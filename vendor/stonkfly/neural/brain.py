...
if sys.platform == "win32":
    LIBRARY = Path(
        os.environ.get("STONKFLY_KERNEL_DLL", r"C:\tools\flyfund-build\memory.dll")
    )
else:
...
def build():
    ...
    if sys.platform == "win32":
        compiler = os.environ.get(
            "STONKFLY_CXX", r"C:\tools\flyfund-winlibs\mingw64\bin\g++.exe"
        )
        # MinGW cannot reliably resolve this workspace's Korean path. Stage the
        # source beside the DLL so the compiler and linker only see ASCII paths.
        staged = LIBRARY.parent / "kernel.cpp"
    ...
    record = {
        ...
        "flags": command[1:-3] if sys.platform == "win32" else command[1:-2],
    }
